---
title: |
  | \Large \bf 민주화 이후 한국 국회의원 재보궐 선거 데이터 코드북
subtitle: |
  | \large Coodbook for General Re/By-Election of South Korea
author: |
...
abstract: |
  | \noindent 1989년 4월 14일부터 2022년 6월 1일까지의 재보궐 146개 선거가 분석대상이며, 이에 대한 자료는 중앙선거관리위원회의 홈페이지의 선거통계시스템(http://info.nec.go.kr)과 중앙선거관리위원회가 편찬한 '대한민국선거사'를 통해 획득하였다. 
output: 
  pdf_document: 
...
: 선거일(DATE), 재보궐선거가 실시된 연-월-일 (1989년 민주화 이후의 시점(1989-04-14)부터 현재(`r format(Sys.time(), '%Y-%b-%d')`)까지 최신화).

```{r, fig.height=7, fig.width=7.5, fig.cap = "재보궐 선거의 추이"}
...
```{r, fig.width=6, fig.height=3.5, fig.cap = "재보궐 선거 사유"}
reby_election %>%
...
```{r, fig.width=6, fig.height=2.5, fig.cap = "재보궐 선거 경쟁률의 분포"}
reby_election %>%
  ...
  geom_density(
    ...
    fill = futurevisions::futurevisions("mars")[1],
    alpha = 0.4
  ) + labs(y = "밀도", x = "재보궐 선거 경쟁률: 100 * (2위 득표율 / 당선자 득표율)")
```

```{r, fig.width=6, fig.height=2.5, fig.cap = "재보궐 시행 사유별 경쟁률"}
reby_election %>%
...
: 재보궐경쟁률(NUM), 1위 당선자 득표율 - 2위 후보자 득표율 (윤종빈 2006; 엄기홍 2012).

```{r, fig.width=6, fig.height=2.5, fig.cap = "재보궐 선거 경쟁률의 분포"}
reby_election %>%
  ggplot(aes(ELEC_COMPETE1)) + 
  geom_density(
    color = futurevisions::futurevisions("mars")[1],
    fill = futurevisions::futurevisions("mars")[1],
    alpha = 0.4
  ) + labs(y = "밀도", x = "재보궐 선거 경쟁률: 당선자 득표율 - 2위 득표율")
```

ELEC_COMPETE2

: 재보궐경쟁률(NUM), (1위 당선자 득표율 - 2위 후보 득표율)/(1위 당선자 득표율 + 2위 후보 득표율) (김지윤 2010).

```{r, fig.width=6, fig.height=2.5, fig.cap = "재보궐 선거 경쟁률의 분포"}
reby_election %>%
  ggplot(aes(ELEC_COMPETE2)) + 
  geom_density(
    color = futurevisions::futurevisions("mars")[1],
    fill = futurevisions::futurevisions("mars")[1],
    alpha = 0.4
  ) + labs(y = "밀도", x = "재보궐 선거 경쟁률: (당선자 득표율 - 2위 득표율)/(당선자 득표율 + 2위 득표율) ")
```

ELEC_COMPETE3

: 재보궐경쟁률(NUM), 1 - ((1위 당선자 득표수 - 2위 후보자 득표수)/(1위 당선자 득표수 + 2위 후보자 득표수)) (황아란 2011).

```{r, fig.width=6, fig.height=2.5, fig.cap = "재보궐 선거 경쟁률의 분포"}
reby_election %>%
  ggplot(aes(ELEC_COMPETE3)) + 
  geom_density(
    color = futurevisions::futurevisions("mars")[1],
    fill = futurevisions::futurevisions("mars")[1],
    alpha = 0.4
  ) + labs(y = "밀도", x = "재보궐 선거 경쟁률: 1 - ((당선자 득표수 - 2위 득표수)/(당선자 득표수 + 2위 득표수))")
```

```{r, results='asis'}
library(corrplot)
reby_election |> dplyr::select(ELEC_COMPETE, ELEC_COMPETE1, ELEC_COMPETE2, ELEC_COMPETE3) |> 
  drop_na() ->
  compete_sets 

compete_sets |> 
  rename(
    선거경쟁도 = ELEC_COMPETE,   선거경쟁도1 = ELEC_COMPETE1,
    선거경쟁도2 = ELEC_COMPETE2, 선거경쟁도3 = ELEC_COMPETE3)  |> 
cor(use = "all.obs", method = "pearson") |> 
  knitr::kable(
      caption = "재보궐경쟁률 측정지표의 상관관계표",
      longtable = T, format = "latex", digits = 2) %>% 
  kableExtra::kable_minimal(latex_options = "HOLD_position")
```

```{r, eval = F, fig.width=8, fig.height=3, fig.cap = "재보궐경쟁률 측정지표의 상관관계"}
compete_sets |> 
  rename(
    선거경쟁도 = ELEC_COMPETE,   선거경쟁도1 = ELEC_COMPETE1,
    선거경쟁도2 = ELEC_COMPETE2, 선거경쟁도3 = ELEC_COMPETE3)  |> 
cor(use = "all.obs", method = "pearson") |> corrplot(method = "number", 
                                                     tl.srt = 360, tl.pos = "lt")
```
...
```{r, fig.width=6, fig.height=2.5, fig.cap = "재보궐 직전 총선 경쟁률의 분포"}
reby_election %>%
...
```{r, fig.width=6, fig.height=2.5, fig.cap = "재보궐 직후 총선 경쟁률의 분포"}
reby_election %>%
...
```{r, fig.width = 8, fig.height=3, fig.cap = "역대 대통령 임기 중 재보궐 선거 수"}
reby_election |> dplyr::select(PRESIDENT) |> group_by(PRESIDENT) |> 
  count() |> 
  mutate(
    PRESIDENT = factor(PRESIDENT,
                       levels = c("노태우", "김영삼", "김대중", "노무현", 
                                  "이명박", "박근혜", "문재인", "윤석열"))) |> 
...
```{r, fig.width=8, fig.height=3.5, fig.cap = "사전투표제도 도입 전후 재보궐선거 투표율 비교(평균)"}
...
```{r, fig.width=8, fig.height=3.5, fig.cap = "사전투표제도 도입 전후 재보궐선거 경쟁률 비교(평균)"}
...
reby_election %>%
  dplyr::select(
    ELEC_VOTER, ELEC_TURNOUT, ELEC_SHARE, RIVAL_SHARE, ELEC_COMPETE, ELEC_GENDER, 
    PRE_RESIGN, PRE_TURNOUT, PRE_COMPETE, ELEC_EFFECNUM, 
    PRE_VOTER, PRE_TURNOUT, PRE_SHARE, PRE_RIVAL_SHARE, PRE_COMPETE, PRE_CANDINUM,
    POST_VOTER, POST_TURNOUT, POST_SHARE, POST_RIVAL_SHARE, POST_COMPETE, POST_CANDINUM,
    ELEC_RULING, ELEC_OPPOSITION
  ) %>%
